...
import controllers.actions.NoEnrolmentRequiredActionISpec.*
...
class NoEnrolmentRequiredActionISpec extends ISpecBase {

  override def applicationBuilder: GuiceApplicationBuilder =
    GuiceApplicationBuilder()
      .appRoutes { app =>
        val noEnrolmentRequiredAction = app.injector.instanceOf[NoEnrolmentRequiredAction]

        { case ("GET", `testPath`) =>
          noEnrolmentRequiredAction { request =>
  ...
  "An endpoint with Auth Action" when {
    ...
    "Auth is successful and returned an HMRC-DSAO-ORG Enrolment" must {
      "pass the action successfully" in {
        ...
        response.body[String] mustBe testSuccessBody(testId)
      }
    }
...
object NoEnrolmentRequiredActionISpec {
  val testPath = "/test-no-enrolment-required-action"
